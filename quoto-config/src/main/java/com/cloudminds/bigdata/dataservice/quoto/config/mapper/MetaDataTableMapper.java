...
public interface MetaDataTableMapper {
    ...
    @Update("update metadata_table set name=#{name},model_level=#{model_level},life_cycle=#{life_cycle},data_domain=#{data_domain},theme_id=#{theme_id},ddl=#{ddl},columns=#{columns,typeHandler=com.cloudminds.bigdata.dataservice.quoto.config.handler.JsonListTypeHandler} where id=#{id}")
