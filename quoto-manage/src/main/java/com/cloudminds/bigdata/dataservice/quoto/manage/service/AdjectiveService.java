...
public class AdjectiveService {
    ...
    public CommonResponse checkUnique(CheckReq checkReq) {
        ...
        if(checkReq.getType()!=1&&checkReq.getType()!=2){
            commonResponse.setSuccess(false);
            commonResponse.setMessage("type只支持1:时间修饰词 2：业务修饰词");
            return commonResponse;
        }
        if (flag == 0) {
            String condition = " and";
            if (checkReq.getType() == 1) {
                condition = condition+" type=1";
            } else {
                condition = condition+" type=2";
                if (checkReq.getDimension_id() == 0) {
                    if (StringUtils.isEmpty(checkReq.getColumn_name())) {
                        commonResponse.setSuccess(false);
                        commonResponse.setMessage("没有关联维度的修饰词,列名不能为空");
                        return commonResponse;
                    }
                    condition = condition+" and column_name="+checkReq.getColumn_name();
                } else {
                    if (dimensionMapper.queryDimensionById(checkReq.getDimension_id()) == null) {
                        commonResponse.setSuccess(false);
                        commonResponse.setMessage("维度不存在");
                        return commonResponse;
                    }
                    condition = condition+" and dimension_id="+checkReq.getDimension_id();
                }

            }
            adjective = adjectiveMapper.findAdjectiveByName(checkReq.getCheckValue(),condition);
        } else if (flag == 1) {
    ...
    public synchronized CommonResponse insertAdjective(Adjective adjective) {
        ...
        if (adjective.getType() == 1) {
            ...
            if (adjective.getDimension_id() == 0) {
                ...
                condition = condition+" and column_name="+adjective.getColumn_name();
            } else {
                ...
                condition = condition+" and dimension_id="+adjective.getDimension_id();
                adjective.setColumn_name(null);
            }

        }
        if(adjectiveMapper.findAdjectiveByName(adjective.getName(),condition)!=null){
            commonResponse.setSuccess(false);
            commonResponse.setMessage("修饰词已存在,请重新命名");
            return commonResponse;
        }
    ...
    public CommonQueryResponse queryAdjective(AdjectiveQuery adjectiveQuery) {
        ...
        if (!StringUtils.isEmpty(adjectiveQuery.getCreator())) {
            condition = condition + " and creator like '%" + adjectiveQuery.getCreator() + "%'";
        }

        if (!StringUtils.isEmpty(adjectiveQuery.getColumn_name())) {
            condition = condition + " and (column_name like '%" + adjectiveQuery.getColumn_name() + "%' or di.`code` like '%" + adjectiveQuery.getColumn_name() + "%')";
        }
