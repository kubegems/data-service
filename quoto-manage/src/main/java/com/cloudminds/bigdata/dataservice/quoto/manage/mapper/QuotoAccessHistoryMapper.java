...
public interface QuotoAccessHistoryMapper {
	@Insert("insert into quoto_access_history(quoto_id,quoto_name,business,domain,process,level,type,success,message) "
			+ "values(#{quoto_id}, #{quoto_name},#{business},#{domain},#{process},#{level},#{type},#{success}, #{message})")
