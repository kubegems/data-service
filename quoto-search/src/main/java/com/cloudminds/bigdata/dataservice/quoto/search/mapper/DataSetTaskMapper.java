...
public interface DataSetTaskMapper {
    ...
    @Insert("insert into data_set_task(name,data_set_id,type,import_type,sync_type,`condition`,cron,advanced_parameters,oozie_hue_uuid,workflow_hue_uuid,waterdrop_hue_uuid,start_time,end_time,create_time,update_time,creator,descr) "
            + "values(#{name},#{data_set_id},#{type},#{import_type},#{sync_type},#{condition},#{cron},#{advanced_parameters},#{oozie_hue_uuid},#{workflow_hue_uuid},#{waterdrop_hue_uuid},#{start_time},#{end_time},now(),now(),#{creator},#{descr})")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    int addDataSetTask(DataSetTask dataSetTask);

    @Update("update data_set_task set name=#{name},data_set_id=#{data_set_id},type=#{type},import_type=#{import_type},sync_type=#{sync_type},`condition`=#{condition},cron=#{cron},advanced_parameters=#{advanced_parameters},oozie_hue_uuid=#{oozie_hue_uuid},workflow_hue_uuid=#{workflow_hue_uuid},waterdrop_hue_uuid=#{waterdrop_hue_uuid},start_time=#{start_time},end_time=#{end_time},descr=#{descr} where id=#{id}")
